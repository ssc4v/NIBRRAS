...
export default function VoiceCallPage() {
  ...
  function createRecognition() {
    ...
    recognition.onend = () => { if (activeRef.current && !mutedRef.current) setTimeout(startListening, 350); };
  ...
  function startListening() {
    ...
    try { const recognition = recognitionRef.current || createRecognition(); recognition?.start(); } catch {}
  }
  ...
  async function speak(text: string) {
    ...
    await new Promise<void>((resolve) => {
      const utterance = new SpeechSynthesisUtterance(text); utterance.lang = 'ar-SA'; utterance.rate = 1;
  ...
  async function startCall() {
    ...
    try {
      ...
      activeRef.current = true; setActive(true); setMuted(false); setLines(prev => [...prev, { role: 'system', text: 'بدأت المكالمة. تكلم الآن.' }]); startListening();
  ...
  async function startCamera() {
    try {
      stopCamera(); const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal: facing } }, audio: false });
      cameraStreamRef.current = stream; if (videoRef.current) videoRef.current.srcObject = stream; setCameraOn(true);
  ...
  function switchCamera() { const next = facing === 'user' ? 'environment' : 'user'; setFacing(next); if (cameraOn) setTimeout(startCamera, 100); }
